<template>
  ...
  >
    <el-form ref="formRef" :model="formData" :rules="formRules" label-width="100px">
      ...
      <template v-if="formData.type === 'single'">
        ...
        <el-form-item label="值班人员">
          <el-button type="primary" plain @click="getRecommendations" :loading="recommending">
        ...
        <el-form-item label="值班地点" prop="location">
          <el-input v-model="formData.location" placeholder="请输入值班地点" clearable />
        </el-form-item>
      </template>
    ...
    <div v-if="previewSchedules.length > 0" class="preview-section">
      ...
      <div class="preview-summary">共生成 {{ previewSchedules.length }} 个班次</div>
    </div>

    <template #footer>
      <span class="dialog-footer">
        <el-button @click="handleClose">取消</el-button>
        <el-button @click="handlePreview" v-if="formData.type !== 'single'"> 预览 </el-button>
        <el-button type="primary" @click="handleSubmit" :loading="submitting"> 确认排班 </el-button>
      </span>
    </template>
  </el-dialog>
</template>
...
const recommendedPersonnel = computed(() => {
  return props.personnel.filter(person => recommendations.value.includes(person.id));
});
...
const filterPersonnel = query => {
  ...
  filteredPersonnel.value = props.personnel.filter(
    person =>
