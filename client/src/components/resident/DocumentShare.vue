  ...
  <div class="document-share">
    <el-form ref="formRef" :model="shareData" label-width="100px">
      ...
      <el-form-item label="分享状态">
        <el-switch v-model="shareData.isShared" active-text="允许分享" inactive-text="禁止分享" />
      </el-form-item>
      ...
      <template v-if="shareData.isShared">
        ...
        <!-- 指定用户 -->
        <el-form-item v-if="shareData.shareScope === 'specific'" label="分享用户" prop="sharedWith">
          <el-select
        ...
        <el-form-item v-if="shareData.expiryType === 'custom'" label="过期时间" prop="expiresAt">
          <el-date-picker
        ...
        <el-form-item v-if="shareData.requirePassword" label="设置密码" prop="password">
          <el-input
    ...
    <div v-if="shareData.isShared && shareLink" class="share-link-section">
      ...
      <div class="share-link">
        <el-input v-model="shareLink" readonly placeholder="生成的分享链接">
          <template #append>
            <el-button @click="copyLink" icon="CopyDocument"> 复制 </el-button>
          </template>
        </el-input>
      </div>
      <div class="qr-code" v-if="showQRCode">
        <el-button @click="generateQRCode" icon="Picture"> 生成二维码 </el-button>
    ...
    <div v-if="shareData.isShared && shareHistory.length > 0" class="share-history">
      ...
      <el-table :data="shareHistory" style="width: 100%">
        ...
        <el-table-column label="操作" width="120">
          <template #default="{ row }">
            <el-button link type="danger" @click="revokeShare(row)" size="small"> 撤销 </el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
    ...
    <div class="share-actions">
      ...
      <el-button type="primary" @click="handleConfirm" :loading="submitting"> 确认分享 </el-button>
    </div>
  </div>
</template>
...
const revokeShare = record => {
  ElMessageBox.confirm('确定要撤销这个分享吗？', '撤销分享', {
