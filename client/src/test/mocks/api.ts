...
export const apiHandlers = [
  ...
  rest.post(`${API_BASE_URL}/auth/login`, (req, res, ctx) => {
    ...
    return res(ctx.status(401), ctx.json({ success: false, message: '用户名或密码错误' }));
  }),

  rest.post(`${API_BASE_URL}/auth/logout`, (req, res, ctx) => {
    return res(ctx.status(200), ctx.json({ success: true, message: '退出成功' }));
  }),
  ...
  rest.get(`${API_BASE_URL}/residents`, (req, res, ctx) => {
    ...
    if (search) {
      filteredResidents = mockResidents.filter(
        r => r.name.includes(search) || r.idCard.includes(search) || r.phone.includes(search)
      );
    }
  ...
  rest.get(`${API_BASE_URL}/residents/:id`, (req, res, ctx) => {
    ...
    if (!resident) {
      return res(ctx.status(404), ctx.json({ success: false, message: '村民不存在' }));
    }

    return res(ctx.status(200), ctx.json({ success: true, data: resident }));
  }),
  ...
  rest.post(`${API_BASE_URL}/residents`, (req, res, ctx) => {
    ...
    return res(ctx.status(201), ctx.json({ success: true, data: newResident }));
  }),
  ...
  rest.put(`${API_BASE_URL}/residents/:id`, (req, res, ctx) => {
    ...
    return res(ctx.status(200), ctx.json({ success: true, data: mockResidents[index] }));
  }),
  ...
  rest.get(`${API_BASE_URL}/committee/:id`, (req, res, ctx) => {
    ...
    if (!committee) {
      return res(ctx.status(404), ctx.json({ success: false, message: '村委不存在' }));
    }

    return res(ctx.status(200), ctx.json({ success: true, data: committee }));
  }),
  ...
  rest.post(`${API_BASE_URL}/feedback`, (req, res, ctx) => {
    ...
    return res(ctx.status(201), ctx.json({ success: true, data: newFeedback }));
  }),
    ...
    return res(
      ...
      ctx.json({
        ...
        data: {
          ...
          monthlyFinance: mockFinanceRecords
            .filter(r => {
