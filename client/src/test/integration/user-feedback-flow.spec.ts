...
describe('用户反馈流程集成测试', () => {
  ...
  describe('反馈提交流程', () => {
    it('应该能够成功提交建议反馈', async () => {
      ...
      const feedbackData = {
        type: 'suggestion',
        title: '关于村口道路维修的建议',
        content: '村口道路破损严重，影响村民出行，建议尽快维修。同时建议加装路灯，提高夜间行车安全。'
      }
  ...
  describe('端到端流程测试', () => {
    it('完整的反馈处理流程', async () => {
      ...
      const newFeedback = feedbackItems.find(item =>
        item.find('[data-testid="feedback-title"]').text() === feedbackData.title
      )
      ...
      // 验证详情信息
      TestHelper.expectElementText(detailWrapper, '[data-testid="feedback-title"]', feedbackData.title)
      TestHelper.expectElementText(detailWrapper, '[data-testid="feedback-content"]', feedbackData.content)
