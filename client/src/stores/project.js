  ...
  const filteredProjects = computed(() => {
    ...
    if (filters.value.riskLevel) {
      filtered = filtered.filter(
        project => project.riskAssessment?.riskLevel === filters.value.riskLevel
      );
    }
    ...
    if (filters.value.startDate) {
      filtered = filtered.filter(
        project => new Date(project.timeline?.startDate) >= new Date(filters.value.startDate)
      );
    }

    if (filters.value.endDate) {
      filtered = filtered.filter(
        project => new Date(project.timeline?.endDate) <= new Date(filters.value.endDate)
      );
    }
