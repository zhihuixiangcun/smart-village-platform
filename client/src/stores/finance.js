...
export const useFinanceStore = defineStore('finance', () => {
  ...
  const filteredExpenses = computed(() => {
    ...
    if (filters.value.startDate) {
      filtered = filtered.filter(
        expense => new Date(expense.expenseDate) >= new Date(filters.value.startDate)
      );
    }

    if (filters.value.endDate) {
      filtered = filtered.filter(
        expense => new Date(expense.expenseDate) <= new Date(filters.value.endDate)
      );
    }
  ...
  const initializeData = async villageId => {
    try {
      ...
      // 并行获取基础数据
      await Promise.all([fetchCategories(), fetchExpenses(), fetchStatistics()]);
    } catch (error) {
