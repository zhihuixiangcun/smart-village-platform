  ...
  <div class="suggestion-review-page">
    ...
    <el-card>
      ...
      >
        ...
        <el-table-column prop="createdAt" label="提交时间" width="150" :formatter="formatDate" />
    ...
    <!-- 评估操作对话框 -->
    <el-dialog v-model="reviewVisible" title="建议评估" width="600px">
      <el-form ref="reviewForm" :model="reviewFormData" :rules="reviewRules" label-width="100px">
    ...
    <!-- 分配对话框 -->
    <el-dialog v-model="assignVisible" title="分配建议" width="400px">
      <el-form :model="assignFormData" label-width="100px">
        <el-form-item label="负责人">
          <el-select
            ...
            style="width: 100%"
          >
            <el-option v-for="user in users" :key="user._id" :label="user.name" :value="user._id" />
          </el-select>
        </el-form-item>
      </el-form>
      <template #footer>
...
export default {
  ...
  setup() {
    ...
    const reviewRules = reactive({
      action: [{ required: true, message: '请选择评估结果', trigger: 'change' }],
      note: [{ required: true, message: '请输入评估意见', trigger: 'blur' }],
    });
