<!-- 内部备注对话框组件 -->
<template>
  <el-dialog v-model="visible" title="添加内部备注" width="500px" :before-close="handleClose">
    <el-form ref="formRef" :model="form" :rules="rules" label-width="100px">
      ...
      <el-form-item label="备注类型" prop="type">
        <el-select v-model="form.type" placeholder="选择备注类型" style="width: 100%">
          <el-option
      ...
      <el-form-item label="相关链接">
        <el-input v-model="form.relatedLink" placeholder="输入相关链接（可选）">
          <template #append>
            <el-button v-if="form.relatedLink" @click="openLink" icon="Link"> 打开 </el-button>
          </template>
        </el-input>
      </el-form-item>
      ...
      <el-form-item label="提醒设置">
        <div class="reminder-settings">
          <el-switch v-model="form.setReminder" active-text="设置提醒" inactive-text="不提醒" />
    ...
    <template #footer>
      ...
      <el-button type="primary" :loading="submitting" @click="submitNote"> 添加备注 </el-button>
    </template>
  </el-dialog>
</template>
...
const rules = computed(() => ({
  type: [{ required: true, message: '请选择备注类型', trigger: 'change' }],
  content: [
